#!/usr/bin/env python
"""Score POG570 patient mutations with local AlphaGenome tissue tracks.
...
It does the following:
  ...
  3. Scores only pancancer driver genes from
     data/driver_genes_coords/Pancancer_1pc.tsv.
  4. Looks up the patient's analysis cohort from
     metadata/pog570_patient_analysis_cohort_mapping.tsv.
...
DEFAULT_OUTPUT_DIR = PROJECT_DIR / "data" / "alphagenome_scores" / "pog570_patient_tissue_tracks_local"
...
def score_patient(client, fasta_extractor, patient_id: str, tissue: str, output_dir: Path) -> None:
    ...
    driver_genes = load_driver_genes("Pancancer", str(PANCANCER_DRIVER_GENE_FILE))
    if GENES:
    ...
    print(f"[{patient_id}] {tissue}: {len(genes)} genes, {len(patient_variants)} hg38 variants")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Score POG570 hg19 patient mutation files on pancancer driver genes."
    )
    ...
    for patient_id in patient_ids:
        ...
        analysis_cohort = cohort_for_patient(patient_id, POG570_PATIENT_COHORT_MAPPING)
        try:
            tissue = tissue_for_cohort(analysis_cohort, POG570_TISSUE_MAPPING)
        except ValueError as exc:
            print(f"Skipping {patient_id}: {exc}", file=sys.stderr)
            continue
        patient_tissues[patient_id] = (analysis_cohort, tissue)

    patient_ids = [patient_id for patient_id in patient_ids if patient_id in patient_tissues]
    ...
    for patient_id in patient_ids:
        analysis_cohort, tissue = patient_tissues[patient_id]
        print(f"Patient: {patient_id}; analysis cohort: {analysis_cohort}; AlphaGenome tissue: {tissue}")
        score_patient(client, fasta_extractor, patient_id, tissue, output_dir)

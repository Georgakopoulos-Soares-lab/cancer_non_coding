#!/bin/bash
#SBATCH --job-name=ag_local
#SBATCH --chdir=/scratch/10900/aksh/OncoGenie
#SBATCH --output=/scratch/10900/aksh/OncoGenie/slurm/logs/ag_local.%j.out
#SBATCH --error=/scratch/10900/aksh/OncoGenie/slurm/logs/ag_local.%j.err
...
# Keep temporary/cache files on scratch instead of home.
...
# Thread limits: one model process, one GPU. Avoid CPU oversubscription.
export OMP_NUM_THREADS="${OMP_NUM_THREADS:-1}"
export MKL_NUM_THREADS="${MKL_NUM_THREADS:-1}"
export OPENBLAS_NUM_THREADS="${OPENBLAS_NUM_THREADS:-1}"
export NUMEXPR_NUM_THREADS="${NUMEXPR_NUM_THREADS:-1}"

# TensorFlow/fsspec/gRPC can fail on some HPC images if the system CA path is
# missing or unreadable. Prefer certifi from the active conda env.
...
SCRIPT="scripts/data_preparation/alphagenome/score_patient_tissue_tracks_alphagenome_local.py"

# Patient-level AlphaGenome tissue-track scoring.
# Override at submit time, for example:
#   sbatch --export=ALL,PATIENT_LIST=metadata/TCGA_patient_list.txt,GENES="TP53 PTEN",BATCH_SIZE=1 scripts/hpc/submit_alphagenome_local_scoring.sh
...
OUTPUT_DIR="${OUTPUT_DIR:-data/alphagenome_scores/patient_tissue_tracks_local}"
...
if [[ "$WEIGHTS_SOURCE" == "huggingface" ]]; then
  if [[ -z "${HF_TOKEN:-}" && -z "${HUGGING_FACE_HUB_TOKEN:-}" && ! -f "${HF_HOME}/token" ]]; then
    ...
    echo "  sbatch --export=ALL,HF_TOKEN=hf_... scripts/hpc/submit_alphagenome_local_scoring.sh" >&2
    exit 2
  fi
fi
...
echo "[alphagenome] genes=${GENES:-all driver genes} max_genes=${MAX_GENES:-none}"

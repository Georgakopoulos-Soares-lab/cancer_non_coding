#!/usr/bin/env python
"""Patient-level AlphaGenome scoring with cancer-specific driver genes.

For each patient, up to the first 30 driver genes are loaded from
data/driver_genes_coords/{CANCER_TYPE}.tsv:
...
Outputs:
  data/alphagenome_scores/patient_tissue_tracks_local_cancer_specific/
...
Examples:
  ...
  python scripts/data_preparation/alphagenome/score_patient_tissue_tracks_alphagenome_local_cancer_specific.py \\
  ...
  python scripts/data_preparation/alphagenome/score_patient_tissue_tracks_alphagenome_local_cancer_specific.py \\
      --patient-list metadata/train_test_split/test_patients.txt \\
      --device gpu \\
      --batch-size 5
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "data"
    / "alphagenome_scores"
    / "patient_tissue_tracks_local_cancer_specific"
)
...
def load_driver_genes(cancer_type: str, driver_gene_file: str | None = None) -> pd.DataFrame:
    path = Path(driver_gene_file) if driver_gene_file else DRIVER_GENE_DIR / f"{cancer_type}.tsv"
    if not path.exists():
        raise FileNotFoundError(f"Missing cancer-specific driver gene file: {path}")
    df = pd.read_csv(path, sep="\t", dtype=str)
    ...
    if "strand" not in df.columns:
        df["strand"] = "."
    return df.head(DEFAULT_MAX_CANCER_DRIVER_GENES).copy()
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional driver gene coordinate TSV override. By default, each patient uses "
            "data/driver_genes_coords/{CANCER_TYPE}.tsv."
        ),
    )
    ...
    parser.add_argument("--batch-size", type=int, default=DEFAULT_BATCH_SIZE, help="Patient-gene completions to ledger at once.")

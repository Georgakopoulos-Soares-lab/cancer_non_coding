#!/usr/bin/env python
"""Patient-level custom-region AlphaGenome scorer-style track scoring.

Independent copy of the local tissue-track scorer. It retains its patient
selection, mutation handling, WT cache, batching, completion files, model
setup, and score format, but only retains ATAC, CHIP_TF, RNA_SEQ, and
SPLICE_SITE_USAGE tracks. ATAC and CHIP_TF are scored in a strand-aware
TSS +/-2 kb promoter window. RNA_SEQ and SPLICE_SITE_USAGE currently use the
annotated gene span; exon/splice-site masks are not yet implemented here.

``--score-region`` is retained for CLI compatibility and does not override
these modality-specific score regions.
...
Outputs:
  data/alphagenome_scores/patient_tissue_tracks_local_custom_regions/
...
DEFAULT_DRIVER_GENE_FILE = DRIVER_GENE_DIR / "Pancancer_1pc.tsv"
TRACK_MAPPING = PROJECT_DIR / "metadata" / "cancer_tissue_alphagenome_tracks.tsv"
DEFAULT_OUTPUT_DIR = PROJECT_DIR / "data" / "alphagenome_scores" / "patient_tissue_tracks_local_custom_regions"
...
CUSTOM_OUTPUT_TYPES = {"RNA_SEQ", "CHIP_TF", "ATAC", "SPLICE_SITE_USAGE"}
DEFAULT_EXCLUDED_OUTPUT_TYPES = []
...
def load_driver_genes(cancer_type: str, driver_gene_file: str | None = None) -> pd.DataFrame:
    ...
    # Driver-gene files may use transcriptional order on the minus strand.
    # Store ascending genomic bounds while retaining strand for TSS scoring.
    bounds = df[["start", "end"]]
    df["start"] = bounds.min(axis=1)
    df["end"] = bounds.max(axis=1)
...
def select_custom_score_interval(interval: genome.Interval, gene_row: pd.Series, output_key: str) -> genome.Interval:
    gene_start = int(gene_row["start"]) - 1
    gene_end = int(gene_row["end"])
    tss = gene_end if str(gene_row.get("strand", "+")) == "-" else gene_start
    if output_key in {"CHIP_TF", "ATAC"}:
        start = max(interval.start, tss - 2_000)
        end = min(interval.end, tss + 2_000)
        if start >= end:
            return None
        return genome.Interval(interval.chromosome, start, end)
    return select_score_interval(interval, gene_row, "gene")
...
def score_patient(client, fasta_extractor, patient_id: str, cancer_type: str, args, output_dir: Path) -> None:
    ...
    cancer_tracks = cancer_tracks[cancer_tracks["output_key"].isin(CUSTOM_OUTPUT_TYPES)].copy()
    ...
    for gene in progress(genes, total=len(genes), desc=f"{patient_id}", enabled=args.show_progress):
        ...
        try:
            ...
            rows = []
            for output_key, output_tracks in tracks.groupby("output_key"):
                region = select_custom_score_interval(interval, gene_rows[gene], output_key)
                if region is None:
                    continue
                label = "tss_plus_2kb" if output_key in {"CHIP_TF", "ATAC"} else "gene"
                rows.extend(summarize_delta(
                    patient_id=patient_id, cancer_type=cancer_type, gene=gene,
                    gene_strand=gene_to_strand.get(gene, ""), interval=interval,
                    score_interval=region, mutations=mutations, ref_to_alt=ref_to_alt,
                    inserted_alt_offsets=inserted_alt_offsets, inserted_ref_anchors=inserted_ref_anchors,
                    wt_output=wt_output, mut_output=mut_output, tracks=output_tracks,
                    aggregation_types=args.aggregation_types, score_region=label,
                ))
        except Exception as exc:
...
def main():
    parser = argparse.ArgumentParser(description="Patient-level custom-region AlphaGenome track-delta scoring.")
    ...
    parser.add_argument(
        ...
        help="Retained for CLI compatibility; modality-specific score regions are always used.",
    )
